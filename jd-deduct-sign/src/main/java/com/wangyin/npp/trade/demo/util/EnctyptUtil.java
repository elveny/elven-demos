...
import com.wangyin.npp.util.DES;
...
public class EnctyptUtil{
	private static Log log = LogFactory.getLog(EnctyptUtil.class);
	...
	public static String signEnvelop(Map<String,String> map,String password,String pirFilePath,String pubFilePath) throws Exception{
		...
		log.info("link="+link);
		byte[] data=link.getBytes(AgencyContants.charset);
		...
		log.info("pfxBase64="+priCert);
		log.info("cerBase64="+pubCert);
	...
	public static String encryptDES(Map<String,String> map,String key) throws Exception{
		String link= SignUtils.map2LinkString(map);//根据请求业务数据key字母排序将数据拼接成字符串
		String encryptData = DES.encrypt(link, key, AgencyContants.charset);
		return encryptData;
	}
	...
	public static Map<String,String> verifySign (Map<String,String> map,String singKey) throws UnsupportedEncodingException{
		...
		//验证签名(必须)
		boolean falg = SignUtils.verify(sign_data,map, sign_type, singKey, AgencyContants.charset);
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public static Map<String,String> verifyMD5Sign (Map<String,String> map,String singKey) throws UnsupportedEncodingException{
		//获取签名类型和签名数据
		String sign_type = map.remove("sign_type").toString();
		String sign_data = map.remove("sign_data").toString();
		//验证签名(必须)
	...
	public static String sign(Map<String, String> paramMap,String signKey) throws UnsupportedEncodingException{
		String sign = SignUtils.sign(paramMap, AgencyContants.singType, signKey, AgencyContants.charset);
		return sign;
	}
